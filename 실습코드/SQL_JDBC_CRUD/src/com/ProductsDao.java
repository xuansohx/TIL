...
public class ProductsDao extends Dao<Integer, Products> {
	...
	public Products select(Integer k, Connection con) throws Exception {
		...
		try {
			...
			rs = psm.executeQuery();
			rs.next();
			result = new Products(rs.getInt("PDNO"), rs.getString("PDNAME"), rs.getString("PDSUBNAME"),
					rs.getString("FACTNO"), rs.getDate("PDDATE"), rs.getInt("PDCOST"), rs.getInt("PDPRICE"),
					rs.getInt("PDAMOUNT"));
		} catch (Exception e) {
			throw e;
		} finally {
	...
	public ArrayList<Products> select(Connection con) throws Exception {
		...
		try {
			...
			rs = psm.executeQuery();
			while (rs.next()) {
				u = new Products(rs.getInt("PDNO"), rs.getString("PDNAME"), rs.getString("PDSUBNAME"),
						rs.getString("FACTNO"), rs.getDate("PDDATE"), rs.getInt("PDCOST"), rs.getInt("PDPRICE"),
						rs.getInt("PDAMOUNT"));
				result.add(u);
			}
	...
	public ArrayList<Join> join(Connection con) throws Exception {
		...
		try {
			...
			while (rs.next()) {
				// u = new
				// Products(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getDate(5),rs.getInt(6),rs.getInt(7),rs.getInt(8));
